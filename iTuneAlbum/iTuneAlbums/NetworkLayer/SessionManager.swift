...
struct SessionManager {
    ...
    mutating func request(url: String, method: HttpMethod = .get, parameters: [String:Any]? = nil,headers: HTTPHeaders? = nil,  completion:@escaping ((_ data:Data?, _ error :Error?, _ responseResult: ResponseResult? ) -> Void)) {
        ...
        guard let request = dataRequest(url: url, headers: headers ?? nil, method: method,parameters: parameters) else { return completion(nil, nil, ResponseResult(code: 404, headerFileds: nil, description: "Failed To Create Request", error: nil)) }
